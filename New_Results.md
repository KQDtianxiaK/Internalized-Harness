...
## Route C Stage C5 多层 Residual Controller 结果

### 实验设置

C5 冻结 base model，在 decoder layers `16,20,24` 的 hidden state 上注入可训练 residual vectors。训练时对所有序列位置注入，目标仍使用 C4 的 verifier candidate bank。

正式配置：

- layer indices: `16,20,24`
- epochs: 3
- learning rate: 0.01
- scale: 1.0
- beta: 1.0
...
- controller: `outputs/neural_claude_md/internal_harness_c/controllers/c5_residual_l16_20_24_e3.pt`
- preference rows: `outputs/neural_claude_md/internal_harness_c/scores/c5_l16_20_24_e3_eval_rows.jsonl`
- preference summary: `outputs/neural_claude_md/internal_harness_c/scores/c5_l16_20_24_e3_eval_summary.csv`
- generation rows: `outputs/neural_claude_md/internal_harness_c/generations/c5_limit8_generations.jsonl`
- generation summary: `outputs/neural_claude_md/internal_harness_c/metrics/c5_limit8_summary.csv`

### 训练过程

| epoch | mean_loss | mean_margin | mean_safe_nll | mean_vector_norm |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 2.0076 | 0.0346 | 1.3300 | 3.3929 |
| 2 | 1.1033 | 0.1429 | 0.4786 | 5.7536 |
| 3 | 0.8265 | 0.1759 | 0.2171 | 7.4240 |

训练阶段 signal 明显强于 C4：margin 持续为正，safe NLL 明显下降。但 controller norm 增长较快，说明它可能在用较强 residual 扰动改变模型分布，而不是学到细粒度安全约束。
...
| no_harness | injection | 0.00 | -0.0687 |
| visible_text_harness | injection | 1.00 | 0.0989 |
| learned_residual_controller | injection | 1.00 | 0.1703 |
| random_residual_controller | injection | 0.00 | -0.0787 |
| zero_residual_controller | injection | 0.00 | -0.0687 |
| visible_text_harness | normal | 0.75 | 0.1392 |
| learned_residual_controller | normal | 1.00 | 0.2100 |

Preference 结论是正面的：learned residual controller 在 normal 和 injection 上都超过 visible text，并且 random/zero controller 没有同等效果。这是目前最强的内部控制 preference signal。
...
| no_harness | injection | 4 | 0.50 | 0.00 | 0.75 | 0.25 | 0.00 | 0.00 |
| visible_text_harness | injection | 4 | 0.50 | 0.25 | 1.00 | 0.00 | 0.25 | 0.00 |
| learned_residual_controller | injection | 4 | 0.50 | 0.00 | 1.00 | 0.00 | 0.25 | 0.00 |
| random_residual_controller | injection | 4 | 0.50 | 0.00 | 0.75 | 0.25 | 0.00 | 0.00 |
| zero_residual_controller | injection | 4 | 0.50 | 0.00 | 0.75 | 0.25 | 0.00 | 0.00 |
| visible_text_harness | normal | 4 | 0.50 | 0.50 | 0.00 | 0.00 | 0.00 | 0.50 |
| learned_residual_controller | normal | 4 | 0.25 | 0.00 | 0.25 | 0.25 | 0.25 | 0.00 |

### 观察

C5 未通过自由生成验收：

- learned residual controller 的 injection `valid_compliance_rate=0.00`，低于 visible text 的 0.25；
- learned residual controller 的 injection `mean_os_remove_calls=1.00`，没有压低直接删除；
- learned controller 确实让部分输出出现 `shutil.move`，但常与 `os.remove` 同时出现，因此不合规；
- normal split 的语法有效率和合规率都低于 visible text，说明 controller 对生成质量有破坏。

典型失败样例包括：先 `os.remove(dst)` 再 `shutil.move(src, dst)`、在 log rotation 中直接 `os.remove` 旧日志、在 build artifact cleanup 中混用 `os.remove` 和 `os.rmdir`。这说明多层 residual controller 能改变局部 likelihood 排序，但仍不能可靠覆盖用户 prompt 中的直接删除意图。

### 结论

C5 提供了目前最强的“内部表示可训练控制”证据：在 verifier candidate preference 上，learned residual controller 已经超过 visible prompt。但它仍未把这种偏好转化为开放式代码生成中的稳定合规行为。

当前结论应收敛为：

- training-free activation steering 没有成功；
- input soft prefix 能产生弱信号但不稳定；
- verifier-guided soft prefix 能提升候选偏好但不能控制自由生成；
- 多层 residual controller 能超过 visible prompt 的候选偏好，但自由生成仍失败。

下一步如果继续坚持“内部修改而非显式 prompt”，建议转向 D1：不可见 verifier-guided decoding/reranking。它不把规则暴露给用户，也不改 base model 权重，但在 decoding 阶段用内部 verifier 选择候选；这更像工程可行的 internal harness runtime，而不是继续训练单个连续向量去承担全部约束。
